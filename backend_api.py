...
def get_products(category=None, search=None):
    ...
    if search: products = [p for p in products if search.lower() in p['name'].lower() or search.lower() in p['brand'].lower()]
    return products
...
def parse_change_data():
    ...
    for row in rows:
        ...
        if s=='Summary':
            m=re.search(r'Total changes detected: (\d+)',c)
            if m: result['total_changes']=max(result['total_changes'],int(m.group(1)))
        elif s=='Change type summary':
            for line in c.split('\n'):
        ...
        elif s=='Most affected locations':
            for line in c.split('\n'):
                line=line.strip('- *').strip()
                if ':' in line:
...
def generate_ai_insights():
    ...
    if changes['total_changes']==0:
        return {"headline":"📭 No marketplace changes detected yet","summary":"Change detection data is empty. Run the GitHub Action to fetch data.","critical_issues":["Trigger the fetch-sheets GitHub Action","Ensure sheets are published"],"actions":[{"priority":"High","action":"Run the GitHub Action manually"}],"key_metrics":{"total_changes":0,"critical_alerts":0,"top_location":"No data","main_risk":"No data"}}
    ...
    return {"headline":f"📊 {total:,} marketplace changes tracked — {main_change['type']} is dominant","summary":f"Latest scan: {total:,} changes. {top_loc['location']} leads with {top_loc['changes']} changes.","critical_issues":[f"{main_change['type']}: {main_change['count']} instances",f"{top_loc['location']}: {top_loc['changes']} changes",f"'{top_kw['keyword']}': {top_kw['changes']} changes"],"actions":[{"priority":"High","action":f"Audit products in {top_loc['location']}"},{"priority":"Medium","action":f"Review '{top_kw['keyword']}' pricing"}],"key_metrics":{"total_changes":total,"critical_alerts":critical,"top_location":top_loc['location'],"main_risk":main_change['type']}}
...
def run_full_diagnosis(df):
    ...
    if "add_to_cart" in df.columns and "checkout_initiated" in df.columns:
        ...
        if atc>0 and ci/atc<0.4:
            ...
            diagnosis["leak_stage_description"]="People add to cart but don't checkout"
    ...
    diagnosis["priority_actions"]=[{"priority":"High","action":"Download CSV template for correct columns"}] if not diagnosis["signal_groups_found"] else [{"priority":"High","action":"Review evidence and check AI Insights tab"}]
...
async def download_template():
    cols=["impressions","clicks","product_views","add_to_cart","checkout_initiated","payment_attempted","orders","selling_price","discount","rating","review_count","delivery_charges","cod_available","payment_success_rate","upi_failures","in_stock_pct","comp_price_change","roas","cpa"]
    output=io.StringIO()
    w=csv.writer(output)
    w.writerow(cols)
...
async def chat(request:ChatRequest):
    changes=parse_change_data()
    if changes['total_changes']>0:
        top_loc=changes['locations'][0] if changes['locations'] else {'location':'Blinkit'}
        return {"answer":f"In the latest scan: {changes['total_changes']:,} changes. Top location: {top_loc['location']}. Check AI Insights for details."}
